...
$check_query = mysqli_query($conn, 'SELECT id, name, locationID from department where locationID = '.$_POST['id']);
if (mysqli_num_rows($check_query)) {
	$output['status']['code'] = "400";
    $output['status']['name'] = "Bad Request";
    $output['status']['description'] = "Department Data still includes this location";
    $output['data'] = [];
    mysqli_close($conn);
    echo json_encode($output);
    exit;
