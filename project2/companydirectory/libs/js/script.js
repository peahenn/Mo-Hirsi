...
function filterPersonnelTable() {
  var filterDepartmentName = $("#Personnelfilter").val().toString();
  var filterLocationName = $("#PersonnelfilterLocation").val().toString();
      //  console.log(filterDepartmentName);
      //  console.log(filterLocationName);
  ...
  $.ajax({
      ...
      data: {
          departmentname: filterDepartmentName,
          locationname: filterLocationName // Add location parameter
      },
      success: function (result) {
          if (result && result.status && result.status.code === '200') {
              populatePersonnelTable(result.data);
          } else {
              console.error('Error filtering personnel data. Server returned:', result);
          }
      },
      error: function (jqXHR, textStatus, errorThrown) {
          console.error('AJAX error:', textStatus, errorThrown);
...
$("#PersonnelfilterModal").on("show.bs.modal", function (e) {
  // Fetch location data when the modal is shown
  fetchLocationsForFilterPersonnel();

  fetchDepartmentsForFilterPersonnel();
});

function fetchDepartmentsForFilterPersonnel() {
  $.ajax({
    url: 'libs/php/getAllDepartments.php',
    type: 'GET',
    dataType: 'json',
...
function fetchLocationsForFilterPersonnel() {
  $.ajax({
    url: 'libs/php/getAllLocation.php',
    type: 'GET',
    dataType: 'json',
    success: function (result) {
      if (result.status.code === '200') {
        ...
        $("#addDepartmentName").val('');
        $("#addLocation").empty(); 
        populateLocationSelection(result.data, "#PersonnelfilterLocation");
...
function getUniqueEmployeeCount(departmentData, departmentName) {
  ...
  $.ajax({
    url: 'libs/php/filterDeleteDepartment.php', 
    type: 'GET', 
  ...
  $.ajax({
    ...
    success: function (result) {
      if (result.status.code === '200') {
        ...
        $('.deleteDepartmentBtn[data-id="' + departmentID + '"]').closest('tr').remove(); 
      } else {
        // Handle other status codes or errors
        console.error('Error: ' + result.status.description);
      }
...
// Setting the select dropwdown back to All

$("#Personnelfilter").change(function () {
  if (this.value > 0) {
      $("#PersonnelfilterLocation").val(0);
     
  }
});

$("#PersonnelfilterLocation").change(function () {
  if (this.value > 0) {
      $("#Personnelfilter").val(0); 
